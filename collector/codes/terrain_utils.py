...
def random_rotated_rectangular_pillar(terrain, _num_obs, _pillar_height):
    ...
    for i in range(num_obs):
        ...
        for rectangular_point in rectangular_points:
            rectangular_point = np.array(rectangular_point)
            rotated_rectangular_point = np.dot(np.linalg.inv(R), rectangular_point) + rand_pos
...
def random_triangular_pillar(terrain, _num_obs, _pillar_height):

    num_obs = _num_obs
    pillar_height_m = _pillar_height # [m]
    pillar_height= pillar_height_m/terrain.vertical_scale # [p]
    pillar_size_m_min = 0.3 # [m]
    pillar_size_m_max = 0.8 # [m]
    pillar_size_min = math.floor(pillar_size_m_min/terrain.horizontal_scale) # [p]
    pillar_size_max = math.floor(pillar_size_m_max/terrain.horizontal_scale) # [p]
    offset = 5 # [deg]
    ...
    for i in range(num_obs):
        ...
        pillar_size_a = random.randint(pillar_size_min, pillar_size_max+1) # [p]
        pillar_size_b = random.randint(pillar_size_min, pillar_size_max+1) # [p]
        pillar_size_c = random.randint(pillar_size_min, pillar_size_max+1) # [p]

        theta_a = random.randint(0+offset,120-offset) # [deg]
        theta_b = random.randint(120+offset,240-offset) # [deg]
        theta_c = random.randint(240+offset,360-offset) # [deg]

        x1 = round(pillar_size_a*np.cos(np.radians(theta_a)) + float(rand_pos[0]))
        y1 = round(pillar_size_a*np.sin(np.radians(theta_a)) + float(rand_pos[1]))
        x2 = round(pillar_size_b*np.cos(np.radians(theta_b)) + float(rand_pos[0]))
        y2 = round(pillar_size_b*np.sin(np.radians(theta_b)) + float(rand_pos[1]))
        x3 = round(pillar_size_c*np.cos(np.radians(theta_c)) + float(rand_pos[0]))
        y3 = round(pillar_size_c*np.sin(np.radians(theta_c)) + float(rand_pos[1]))

        triangle_points = [np.array([x1, y1]), np.array([x2, y2]), np.array([x3, y3])]

        triangle_points = np.array(triangle_points, dtype=np.int32).reshape((3,2))
        rr, cc = polygon(triangle_points[:, 0], triangle_points[:, 1])

        # _rr = rr.reshape((-1,1))
        # _cc = cc.reshape((-1,1))
        # _triangle_points = np.concatenate((_rr, _cc), axis=1)
       
        # if not np.any(_triangle_points == rand_pos):
        #     print("skip")
        #     continue
        # else:    
        try:
...
def random_polygon_pillar(terrain, _num_obs, _pillar_height):
    ...
    for i in range(num_obs):
        ...
        for i in list(range(0, side_num)):
            len = random.randint(pillar_size_min, pillar_size_max)
            theta_dis = round(360/side_num)
            theta = random.randint(theta_dis*i+offset, theta_dis*(i+1)-offset)
            
            polygon_point_x = round(len*np.cos(np.radians(theta)) + float(rand_pos[0]))
            polygon_point_y = round(len*np.sin(np.radians(theta)) + float(rand_pos[1]))

            polygon_points.append([polygon_point_x, polygon_point_y])

        polygon_points = np.array(polygon_points, dtype=np.int32).reshape((side_num,2))

        rr, cc = polygon(polygon_points[:, 0], polygon_points[:, 1])
...
def random_wall_pillar(terrain,_num_obs, _pillar_height):
