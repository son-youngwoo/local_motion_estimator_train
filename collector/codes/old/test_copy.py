...
while True:
    ...
    if cnt == 60: # initialize
        ...
        for data in dataset:
            if data.area_state == 1:
                for_states = gym.get_actor_dof_states(envs[i], actor_handles[i], gymapi.STATE_ALL)
                dof_positions = dof_states['pos']
        ...
        print(gym.get_env_count(sim)) 
    
    # elif (cnt == 400):
    #     # for i in range(len(envs)):
    #     #     env_handle = gym.get_env(sim, i)
    #     #     gym.destroy_env(env_handle)

    #     for i in envs:
    #         print(i)
    #         gym.destroy_env(i)
    #         print(gym.destroy_env(i))
