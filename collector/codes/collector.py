...
class Data:
    ...
    def getLabel(_data, _num_image, _base_path):
        ...
        data = [_data.id + _num_image, _data.x_init_world, _data.y_init_world, _data.yaw_init_world, _data.x_tar_world, _data.y_tar_world, _data.yaw_tar_world, _data.isSuccess]
...
# base_path = "../../dataset/random_ellipse/"

# ver2
base_path = "../../dataset/ver2/random_rectangular/"
# base_path = "../../dataset/ver2/random_polygon/"
# base_path = "../../dataset/ver2/random_ellipse/"
...
while True:
    ...
    if process_flag == 0:
        for i in range(len(envs)):
            ...
            # target yaw
            yaw_tar_deg = random.randint(0,360) #init_pose[iteration]
            ...
            dataset[i].yaw_tar_world = yaw_tar_deg
        ...
        ## step2
            ...
            for i in success_envs1:
                yaw_tar_rad = np.deg2rad(dataset[i].yaw_tar_world)

                quat_tar = rotate_z_quaternion(yaw_tar_rad)

                print("yaw_tar_world : ",dataset[i].yaw_tar_world)
                
                state = gym.get_actor_rigid_body_states(envs[i], actor_handles[i], gymapi.STATE_ALL)
                state['pose']['p'].fill((dataset[i].x_init_world, dataset[i].y_init_world, init_z))
                state['pose']['r'][0].fill((quat_tar[3], quat_tar[2], quat_tar[1], quat_tar[0]))
            ...
            for i in success_envs1:
                body_states = gym.get_actor_rigid_body_states(envs[i], actor_handles[i], gymapi.STATE_POS)
                body_positions = body_states['pose']['p']
                if body_positions[2][2] < pillar_height: # step2 success
                    success_envs2.append(i)
                else:
                    dataset[i].isSuccess = 0

        ## step3
        elif cnt == 120:
            for i in success_envs2:
